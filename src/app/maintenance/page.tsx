...
import { getMaintenanceRequestsAction } from '@/app/api/server-actions/maintenance-actions';
import { getComplexesAction, getBuildingsAction } from '@/app/api/server-actions/housing-actions';
import { MaintenanceRequest, HousingComplex, Building } from '@/models/types';
...
export default function MaintenanceRequestsPage() {
  ...
  useEffect(() => {
    const fetchComplexes = async () => {
      try {
        const result = await getComplexesAction();
        if (result.error) {
  ...
  useEffect(() => {
    if (filters.complexId) {
      const fetchBuildings = async () => {
        try {
          const result = await getBuildingsAction(filters.complexId);
          if (result.error) {
    ...
    const fetchMaintenanceRequests = async () => {
      ...
      try {
        ...
        const filteredRequests = filters.priority 
          ? result.data.filter(request => request.priority === filters.priority)
          : result.data;
