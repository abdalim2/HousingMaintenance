...
async function POST(request) {
    try {
        ...
        if (action === 'setup_database') {
            ...
            const setupResult = await runSetupDBScript();
...
async function runSetupDBScript() {
    return new Promise((resolve)=>{
        try {
            const scriptPath = __TURBOPACK__imported__module__$5b$externals$5d2f$path__$5b$external$5d$__$28$path$2c$__cjs$29$__["default"].resolve(process.cwd(), 'scripts', 'init-pg-db.js');
            ...
            child.stdout.on('data', (data)=>{
                ...
                console.log(`Setup script output: ${text}`);
                output += text;
            });
            child.stderr.on('data', (data)=>{
                const text = data.toString();
                console.error(`Setup script error: ${text}`);
                errorOutput += text;
            });
            child.on('close', (code)=>{
                if (code === 0) {
                    resolve({
                        success: true,
                        details: output || 'Setup script executed successfully'
                    });
                } else {
                    resolve({
                        success: false,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__34cef9de._.js.map